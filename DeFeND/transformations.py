...
class DenseTransforms:
    def __init__(self,
        ...
        self.color_jitter = torchvision.transforms.ColorJitter(
            0.4 * jitter_strength, 0.4 * jitter_strength, 0.4 * jitter_strength,
            0.4 * jitter_strength
        )
        ...
        color_transform.append(torchvision.transforms.RandomApply([blur], p=0.1))
        self.color_transform = torchvision.transforms.Compose(color_transform)

        # Construct final transforms
        normalize = torchvision.transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        self.final_transform = torchvision.transforms.Compose([torchvision.transforms.ToTensor(), torchvision.transforms.Lambda(lambda x: x.repeat(3, 1, 1)), normalize])
    ...
    def __call__(self, sample):
        multi_crops = []
        ## reshape the sample to a 3d pil image by repeating the image 3 times
