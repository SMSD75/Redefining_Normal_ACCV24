...
import numpy as np
from vision_transformers import leo_vit_small, leo_vit_base
...
class Leopart():
    ...
    def __init__(self, gpus: int, num_samples: int, batch_size: int, max_epochs: int, lr_heads: float,
                 ...
                 arch: str = 'vit-small', device: str = 'cuda'):
        """
    ...
    def init_model(self):
        ...
        if self.arch == 'vit-small':
            model_func = leo_vit_small
        elif self.arch == 'vit-base':
            model_func = leo_vit_base
        else:
    ...
    def load_state_dict(self, state_dict, strict=True):
        msg = self.model.load_state_dict({".".join(k.split(".")[1:]): v for k, v in state_dict[0].items()}, strict=strict)
        print(msg)
        if self.use_teacher:
            msg = self.teacher.load_state_dict({".".join(k.split(".")[1:]): v for k, v in state_dict[1].items()}, strict=strict)
            print(msg)
    ...
    def configure_optimizers(self):
        ...
        for name, param in self.model.named_parameters():
            if name.startswith("projection_head")  or name.startswith("prototypes"):
                ...
                ## if the name is in the last two block
        ...
        if self.optim == 'adamw':
            self.optimizer = torch.optim.AdamW(params, weight_decay=self.weight_decay)
        else:
            raise ValueError(f'Optimizer {self.optim} not supported')
        self.scheduler = CosineAnnealingLR(self.optimizer, T_max=self.train_iters_per_epoch * self.max_epochs,
    ...
    def training_step(self, batch: Tuple[List[torch.Tensor], Dict]) -> float:
        ...
        if self.use_teacher:
            ...
            with torch.no_grad():
                m = self.momentum_schedule[self.global_step]  # momentum parameter
                for param_q, param_k in zip(self.model.parameters(), self.teacher.parameters()):
                    param_k.data.mul_(m).add_((1 - m) * param_q.detach().data)

        # print('lr_backbone', self.optimizer.param_groups[0]['lr'])
        # print('lr_heads', self.optimizer.param_groups[2]['lr'])
        # self.log('weight_decay', self.optimizers().param_groups[0]['weight_decay'], on_step=True, on_epoch=False)
        # self.log('train_loss', loss, on_step=True, on_epoch=False)
        ## check if mlp head has gradient
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer_step()
        return loss
